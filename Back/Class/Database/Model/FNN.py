...
class FNN(ModelAbstract.Model):
    def __init__(self):
        self.fnnCh = fnn("C:/LIBS/Back/Class/Database/Model/FNN/DYNAMIC_CHARGE_BID001_BID002_BID003_20-20-2.keras")
        self.fnnDch = fnn('C:/LIBS/Back/Class/Database/Model/FNN/DYNAMIC_BID001_BID002_BID003_20-20-2.keras')

    def toString(self):
        return "FNN"
    def runOneStep(self, z, u, charge):
        if(charge == 1):
            retour= self.fnnCh.runOneStepDynamicOnline(z,u)
            return float(retour[0][1]),float(retour[0][0])
        if(charge == 0):
            retour= self.fnnDch.runOneStepDynamicOnline(z,u)
            return float(retour[0][1]),float(retour[0][0])
###################################################################
...
def createModel(entree,sortie,epochs,name):
    ...
    model.add(keras.layers.Flatten(input_shape=(20, 2)))
